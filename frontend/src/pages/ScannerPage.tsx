...
import type { ScannerResult, Signal, SignalWithExplanation } from "../lib/types";
...
function AIAnalysisDrawer({
  ...
  const [data, setData] = useState<SignalWithExplanation | null>(null);
          ...
          {data?.ai_explanation && (
            <div className="space-y-4 rounded-md border border-line bg-ink/60 p-4">
              {[
                ...
                { label: "Confidence Commentary", key: "confidence_commentary" as const },
                ...
                <div key={key}>
                  ...
                  <p className="text-sm leading-6 text-slate-300">{data.ai_explanation[key]}</p>
                </div>
              ))}
              <p className="text-right text-xs text-slate-600">Model: {data.ai_explanation.model}</p>
            </div>
          )}
        </div>
      </div>
